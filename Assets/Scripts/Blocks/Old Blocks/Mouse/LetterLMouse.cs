...
public class LetterLMouse : MonoBehaviour
{
    [SerializeField]
    private Transform letterLPlace;
    ...
    private void OnMouseUp()
    {
        ...
        {
            if (Mathf.Abs(transform.position.x - letterLPlace.position.x) <= 0.5f &&
    Mathf.Abs(transform.position.y - letterLPlace.position.y) <= 0.5f)
            {
                transform.position = new Vector2(letterLPlace.position.x, letterLPlace.position.y);
                pressed = false;
                locked = true;
            }
            else
