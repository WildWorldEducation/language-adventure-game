...
public class LetterRMouse : MonoBehaviour
{

    [SerializeField]
    private Transform letterRPlace;
    ...
    private void OnMouseUp()
    {
        ...
        {
            if (Mathf.Abs(transform.position.x - letterRPlace.position.x) <= 0.5f &&
                 Mathf.Abs(transform.position.y - letterRPlace.position.y) <= 0.5f)
            {
                transform.position = new Vector2(letterRPlace.position.x, letterRPlace.position.y);
                pressed = false;
                locked = true;
            }
            else
