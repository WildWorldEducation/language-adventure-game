...
public class LetterWMouse : MonoBehaviour
{

    [SerializeField]
    private Transform letterWPlace;
    ...
    private void OnMouseUp()
    {
        ...
        if (sceneName == "OwlExercise")
        {
            ...
            if (Mathf.Abs(transform.position.x - letterWPlace.position.x) <= 0.5f &&
    Mathf.Abs(transform.position.y - letterWPlace.position.y) <= 0.5f)
            {
                transform.position = new Vector2(letterWPlace.position.x, letterWPlace.position.y);
                pressed = false;
                locked = true;
            }
            else
